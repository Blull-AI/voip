...
VoIP server** and makes/receives real phone calls. It provides SIP over secure
WebSocket + WebRTC behind a small, typed API: `connect`,
...
app.get("/api/voip/credentials", requireSession, async (req, res) => {
  ...
  const response = await fetch(
    `${process.env.BLULL_API_URL}/voip/credentials`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok) {
    return res
      ...
      .json({ error: "SIP credentials unavailable" });
  }
...
| `logLevel`             | `'debug' \| 'warn' \| 'error' \| 'silent'` | `'warn'`                | Controls SDK log verbosity.          |
...
- **Browser-only.** WebRTC/`getUserMedia`/WebSocket run in the browser. The
  package is SSR-safe to _import_ (Next.js RSC/SSR); the DOM-touching calls throw
